...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=correct_label)) + sum(reg_losses)
...
"""
def process_vid(input_image):
    
    img = cv2.imread(input_image)
    im_shape = img.shape
    output_image = helper.return_inference_single_image(sess, im_shape, logits, keep_prob, input_image)
    return output_image
"""
